jQuery.extend({
    ...
    createUploadForm: function (id, fileElementId, data) {//id为当前系统时间字符串，fileElementId为页面<input type='file' />的id，data的值需要根据传入json的键来决定
        ...
        if (data) {//通常为false
        	//alert(data);
            //for (var i in data) {
                jQuery('<input type="hidden" name="users" value="' + data + '" />').appendTo(form); //根据data的内容，创建隐藏域，这部分我还不知道是什么时候用到。估计是传入json的时候，如果默认传一些参数的话要用到。
            //alert(i);
            //}
            ...
            try {                if (io.contentWindow) {//动态iframe所在窗口对象是否存在
                ...
                var status;                try {
                   ...
                   // alert(status);
                    ...
                    if (status != "error") {    
                    	//alert("asdsf");// process the data (runs the xml through httpData regardless of callback)
